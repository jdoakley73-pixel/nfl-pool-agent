...
from yahoo_connection_ui import render_yahoo_connection

ROSTER_VERSION = "2026-week1-nailor-v3"
DEFAULT_ROSTER = [
    ...
    {"Player": "Jalen Nailor", "Pos": "WR", "Team": "LV", "Slot": "BN", "Status": "Bench"},
...
def render_weekly_gm():
    ...
    m1.metric("Active", len(active)); m2.metric("Starters", len(starters)); m3.metric("Bench", len(bench)); m4.metric("IR", len(ir))
    ...
    if missing:
        ...
        st.success("✅ All 9 starting slots filled.")

    st.markdown("#### GM action board")
    st.warning("⚠️ WATCH: Patrick Mahomes and Rome Odunze are tagged Q. Recheck status before their games.")
    st.info("🩼 IR: Jordyn Tyson remains stashed without consuming an active roster spot.")
    ...
    display = roster.copy(); display["_order"] = display["Slot"].map(lineup_order).fillna(99)
    ...
    st.markdown("#### Manual roster manager")
    st.caption("Fallback editor until the live Yahoo roster parser is activated.")
    edited = st.data_editor(
        roster, use_container_width=True, hide_index=True, num_rows="dynamic",
        column_config={
        ...
        }, key="weekly_roster_editor_inline_v3",
    )
    c1, c2 = st.columns(2)
    if c1.button("💾 Save roster", use_container_width=True, type="primary", key="save_weekly_roster_inline_v3"):
        st.session_state.weekly_roster = edited.copy(); st.rerun()
    if c2.button("↩️ Restore snapshot", use_container_width=True, key="reset_weekly_roster_inline_v3"):
        st.session_state.weekly_roster = pd.DataFrame(DEFAULT_ROSTER); st.rerun()

    with st.expander("Starters"):
        st.dataframe(starters[["Player", "Pos", "Team", "Slot"]], use_container_width=True, hide_index=True)
    with st.expander("Bench + IR"):
    ...
    st.caption("Yahoo API access is read-only; transactions and lineup changes remain in Yahoo.")
